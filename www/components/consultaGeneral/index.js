app.consultaG = kendo.observable({
    onShow: function () {
        ...
        llamarNuevoestilo("btnBusquedasConsulta");
        llamarNuevoestiloIconB("icnBusquedasConsulta")
            ...
            if (window.navigator.simulator === true) {
                ...
                cordova.plugins.barcodeScanner.scan(
                    function (result) {
                        ...
                        if (!result.cancelled) {
                            alert(result.text);
                            $("#infoPlacasVINAE").val(result.text);
                            document.getElementById("infoPlacasVINAE").innerHTML = result.text;
